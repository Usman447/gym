                    ...
                    <div class="panel no-border">
                        ...
                        <div class="panel-body">
                            ...
                            <div id="itemsContainer">
                                @foreach($foodOrder->orderItems as $index => $orderItem)
                                <div class="row">
                                    <div class="col-sm-5">
                                        <div class="form-group item-id">
                                            <label>Item</label>
                                            <select name="items[{{ $index }}][id]" class="form-control selectpicker show-tick show-menu-arrow item-select" data-live-search="true">
                                                <option value="">-- Select Item --</option>
                                                <optgroup label="Food Items">
                                                    @foreach($foodItems as $foodItem)
                                                        <option value="food_{{ $foodItem->id }}" data-type="food" data-price="{{ $foodItem->amount }}" {{ ($orderItem->food_item_id == $foodItem->id) ? 'selected' : '' }}>{{ $foodItem->name }} - ₹{{ number_format($foodItem->amount, 0) }}</option>
                                                    @endforeach
                                                </optgroup>
                                                <optgroup label="Inventory Items">
                                                    @foreach($inventoryItems as $inventoryItem)
                                                        <option value="inventory_{{ $inventoryItem->id }}" data-type="inventory" data-price="{{ $inventoryItem->amount }}" data-quantity="{{ ($orderItem->inventory_id == $inventoryItem->id) ? ($inventoryItem->quantity + $orderItem->quantity) : $inventoryItem->quantity }}" {{ ($orderItem->inventory_id == $inventoryItem->id) ? 'selected' : '' }}>{{ $inventoryItem->name }} - ₹{{ number_format($inventoryItem->amount, 0) }}</option>
                                                    @endforeach
                                                </optgroup>
                                            </select>
                                            <small class="inventory-quantity-info text-muted" style="display: {{ $orderItem->inventory_id ? 'block' : 'none' }};">Available Quantity: <span class="available-quantity">{{ $orderItem->inventory_id && $orderItem->inventory ? ($orderItem->inventory->quantity + $orderItem->quantity) : 0 }}</span></small>
                                        </div>
                                    </div>
                                    <div class="col-sm-3">
                                        <div class="form-group">
                                            <label>Quantity</label>
                                            <input type="number" name="items[{{ $index }}][quantity]" value="{{ $orderItem->quantity }}" min="1" class="form-control item-quantity" onchange="calculateTotal()">
                                    ...
                                    <div class="col-sm-3">
                                        <div class="form-group">
                                            <label>Amount</label>
                                            <input type="text" class="form-control item-amount" readonly value="{{ $orderItem->item_amount * $orderItem->quantity }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-1">
                                        <div class="form-group">
                                            <label>&nbsp;</label>
                                            <span class="btn btn-sm btn-danger pull-right {{ $index == 0 ? 'hide' : '' }} remove-item">
                                    ...
                                    <div class="panel no-border bg-blue-grey-50">
                                        <div class="panel-body">
                                            <div class="row">
                                                ...
                                                <div class="col-sm-6">
                                                    <h3 class="no-margin">Rs. <span id="totalAmount">{{ number_format($foodOrder->total_amount, 0) }}</span></h3>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
@section('footer_scripts')
    <script type="text/javascript">
        var itemCounter = {{ $foodOrder->orderItems->count() - 1 }};
        var maxItems = 100;
            ...
            $('#addItem').click(function () {
                if (itemCounter < maxItems) {
                    ...
                    $clonedRow.find('select, input').each(function() {
                        ...
                        if (name) {
                            name = name.replace(/\[\d+\]/, '[' + itemCounter + ']');
                            $this.attr('name', name);
                        }
                        var id = $this.attr('id');
                        if (id) {
                            id = id.replace(/_(\d+)$/, '_' + itemCounter);
                            $this.attr('id', id);
                        }
                    });
        ...
        function validateInventoryQuantity($row, requestedQuantity, availableQuantity) {
            ...
            if (requestedQuantity > availableQuantity) {
                ...
                // Only clear error if it's not a duplicate error
                if ($errorMsg.text().indexOf('Total quantity') === -1) {
                    $quantityInput.removeClass('has-error');
                    $errorMsg.hide();
                    $quantityInput.css('border-color', '');
